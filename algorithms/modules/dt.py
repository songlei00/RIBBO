...
class DecisionTransformer(GPT2):
    ...
    def forward(
        ...
        regret_embedding = self.regret_embed(self.regret_embed(regrets_to_go), timesteps)
        
        if key_padding_mask is not None:
            key_padding_mask = torch.stack([key_padding_mask, key_padding_mask, key_padding_mask], dim=2).reshape(B, 3*L)
        
        stacked_input = torch.stack([regret_embedding, x_embedding, y_embedding], dim=2).reshape(B, 3*L, X)
